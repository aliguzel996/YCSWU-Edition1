...
const DEVICE_MARGIN_LEFT = 0;
const DEVICE_MARGIN_RIGHT = 0;
const DEVICE_MARGIN_TOP = 0;
const DEVICE_MARGIN_BOTTOM = 0;
const DEVICE_WINDOW_WIDTH = DEVICE_MARGIN_LEFT + DEVICE_FRAME_WIDTH + DEVICE_MARGIN_RIGHT;
const DEVICE_WINDOW_HEIGHT = DEVICE_MARGIN_TOP + DEVICE_FRAME_HEIGHT + DEVICE_MARGIN_BOTTOM;
...
function registerIpcHandlers() {
  ...
  ipcMain.handle("special:minimize-window", async () => {
    mainWindow?.minimize();
    return { ok: true };
  });
  ipcMain.handle("special:close-window", async () => {
    mainWindow?.close();
    return { ok: true };
  });
